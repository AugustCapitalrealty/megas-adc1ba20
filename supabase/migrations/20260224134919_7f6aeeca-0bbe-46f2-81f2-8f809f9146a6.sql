...
CREATE OR REPLACE FUNCTION public.get_solicitacoes_backoffice(p_status request_status DEFAULT NULL::request_status, p_empreendimento empreendimento DEFAULT NULL::empreendimento, p_search text DEFAULT NULL::text, p_limit integer DEFAULT 100, p_offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, protocolo text, tipo text, status request_status, empreendimento empreendimento, valor numeric, created_at timestamp with time zone, updated_at timestamp with time zone, descricao text, emergencial boolean, numero_chamado_fluig text, numero_projuris text, fornecedor_cnpj text, fornecedor_razao text, solicitante_nome text, solicitante_email text, cliente_nome text, total_anexos bigint, total_docs_fiscais bigint, total_docs_emitidos bigint, ultima_atualizacao_status timestamp with time zone, data_pendente_correcao timestamp with time zone, fornecedor_email_contato text, fornecedor_telefone_contato text)
 LANGUAGE sql
