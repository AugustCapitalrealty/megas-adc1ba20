-- Add numero_projuris column to solicitacoes table
ALTER TABLE public.solicitacoes ADD COLUMN IF NOT EXISTS numero_projuris text;

-- Drop and recreate the RPC function with numero_projuris
DROP FUNCTION IF EXISTS public.get_solicitacoes_backoffice(request_status, empreendimento, text, integer, integer);

CREATE FUNCTION public.get_solicitacoes_backoffice(
