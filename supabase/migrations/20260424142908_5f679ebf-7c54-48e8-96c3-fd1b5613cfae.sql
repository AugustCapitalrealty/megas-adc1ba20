-- Fix get_solicitacoes_backoffice: COALESCE skips NULL but not empty strings.
-- Use NULLIF + TRIM so suppliers with empty nome_fantasia fall back to razao_social.
...
RETURNS TABLE(
  ...
  fornecedor_email_contato text, fornecedor_telefone_contato text
)
LANGUAGE sql
