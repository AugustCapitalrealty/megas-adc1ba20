-- Corrigir a função calcular_sla_solicitacao
...
DECLARE
  ...
  sol RECORD;
  status_atual TEXT;
  -- Status após OC emitida que devem encerrar o SLA do backoffice
...
BEGIN
  ...
  LOOP
    -- Se Fluig já foi lançado uma vez, ignora qualquer evento posterior:
    -- o SLA do backoffice termina definitivamente nesse marco.
    ...
    -- Pausa por pendências do solicitante
    ...
    -- Solicitante retornou: backoffice volta a atender
    IF rec.status_anterior IN ('pendente_correcao', 'aguardando_informacoes') 
      ...
      -- IMPORTANTE: NÃO zerar tempo_backoffice nem data_fluig_rm aqui.
      -- Quando há retomada, o tempo continua de onde parou.
    END IF;
    ...
    -- Marco principal: Fluig/RM lançado → SLA do backoffice encerra
    ...
    -- Em processamento também encerra o atendimento do backoffice
    ...
    -- Status pós-OC: trabalho do backoffice acabou
    IF rec.status_novo = ANY(status_pos_oc) THEN
      IF em_contagem AND data_inicio IS NOT NULL THEN
        tempo_backoffice := tempo_backoffice + calcular_horas_uteis(data_inicio, rec.created_at);
      END IF;
      em_contagem := FALSE;
  ...
  -- Acumula o que falta apenas se o status atual ainda exige ação do backoffice
...
-- Atualizar a timeline para marcar os novos pontos de "fim"
